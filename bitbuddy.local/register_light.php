...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        $error = 'Пароль должен быть не менее 8 символов, содержать цифру и заглавную букву';
    } else {
...
<button onclick="location.href='register.php'" class="text-slate-500 hover:text-slate-900 transition-colors duration-300 focus:outline-none" title="Переключить на темную тему">
...
<h1 class="text-3xl font-extrabold tracking-tight text-slate-900 mb-2">Регистрация</h1>
<p class="text-on-surface-variant text-sm">Создайте аккаунт для доступа к BitBuddy.</p>
</div>
<!-- Tab Switcher -->
<div class="flex p-1 bg-slate-100 rounded-lg mb-8 relative">
<div class="absolute inset-y-1 right-1 w-[calc(50%-4px)] bg-white backdrop-blur-md border border-slate-200 rounded-md shadow-sm pointer-events-none"></div>
<a href="login_light.php" class="flex-1 py-2 text-sm font-medium text-on-surface-variant hover:text-slate-900 transition-colors duration-300 z-10 relative focus:outline-none text-center">
Вход
</a>
...
<input name="username" class="w-full bg-slate-50 text-on-surface pl-12 pr-4 py-3 rounded-xl border border-slate-200 focus:border-primary focus:ring-0 focus:outline-none transition-all placeholder:text-slate-400" id="username" placeholder="username" type="text" required/>
</div>
</div>
<!-- Email Field -->
<div>
...
<input name="email" class="w-full bg-slate-50 text-on-surface pl-12 pr-4 py-3 rounded-xl border border-slate-200 focus:border-primary focus:ring-0 focus:outline-none transition-all placeholder:text-slate-400" id="email" placeholder="name@example.com" type="email" required/>
</div>
</div>
<!-- Password Field -->
<div>
...
<input name="password" class="w-full bg-slate-50 text-on-surface pl-12 pr-12 py-3 rounded-xl border border-slate-200 focus:border-primary focus:ring-0 focus:outline-none transition-all placeholder:text-slate-400" id="password" placeholder="Мин. 8 символов, цифра и заглавная буква" type="password" required/>
